...
function lottery() {
    ...
    for (let i=0; i<10; i++){
        ...
        filtered_numbers.push(numbers[random]);
        numbers.splice(random, 1)
    }
    for(i=0; i<10; i++){
        for(j=i+1; j<10; j++){
            if(filtered_numbers[i] > filtered_numbers[j]){
                temp = filtered_numbers[i];
                filtered_numbers[i]=filtered_numbers[j];
                filtered_numbers[j]=temp;
            }
        }
    }
    for(let i=0; i<5; i++){
        temp_numbers = filtered_numbers.slice();
        for(let j=0; j<6; j++){
            random = Math.floor(Math.random() *  temp_numbers.length);
            double_ary[i][j] = temp_numbers[random];
            temp_numbers.splice(random, 1)
        }
    ...
    let fn_st = "추출된 번호 : "+filtered_numbers;
    document.getElementById("filtered-num").innerText = fn_st;
